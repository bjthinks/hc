...
expressionDisplayTests = test [
  ...
  testDisplay "factor(f(x^2+2*x+1)+f(y^2+2*y+1))"
    "f((x + 1)^2) + f((y + 1)^2)"
  , testDisplay "a(2)(3)b" "3 b a(2)"
  , testDisplay "2(a)(3)b" "6 a b"
  , testDisplay "a/b c" "a / b c"
...
expressionDisplayTests' = test [
  ...
  , testDisplay' "a-b" "a - b"
  , testDisplay' "(a-b)-c" "a - b - c"
  , testDisplay' "a-(b-c)" "a - (b - c)" -- TODO this is stupid output
  , testDisplay' "a*b" "a b"
  , testDisplay' "a b" "a b"
  , testDisplay' "a*(b + c)" "a (b + c)"
  , testDisplay' "a*(b-c)" "a (b - c)"
...
{-
  ...
  "a / b - c / d"
  , testDisplay' (ASTProduct (ASTSum a b) (ASTSum c d)) "(a + b) (c + d)"
  , testDisplay' (ASTProduct (ASTDifference a b) (ASTDifference c d))
  "(a - b) (c - d)"
  , testDisplay' (ASTQuotient (ASTSum a b) (ASTSum c d)) "(a + b) / (c + d)"
  , testDisplay' (ASTQuotient (ASTDifference a b) (ASTDifference c d))
  "(a - b) / (c - d)"
  , testDisplay' (ASTPower a b) "a^b"
  , testDisplay' (ASTPower a (ASTPower b c)) "a^b^c"
  , testDisplay' (ASTPower (ASTPower a b) c) "(a^b)^c"
  , testDisplay' (ASTProduct (ASTPower a b) (ASTPower c d)) "a^b c^d"
  , testDisplay' (ASTPower (ASTProduct a b) (ASTProduct c d)) "(a b)^(c d)"
  , testDisplay' (ASTSum a (ASTNegation b)) "a + -b"
  , testDisplay' (ASTSum (ASTNegation a) b) "-a + b"
  , testDisplay' (ASTDifference a (ASTNegation b)) "a - -b"
  , testDisplay' (ASTDifference (ASTNegation a) b) "-a - b"
  , testDisplay' (ASTProduct a (ASTNegation b)) "a (-b)"
...
tests = test ["Tokenizer" ~: test_Tokenizer,
              ...
              "new expression display" ~: expressionDisplayTests',
